...
def sliding_windows(binary_warped, nwindows=9):
    ...
    try:	
        ...
        right_fit = 0
        val = 0
    return left_fit, right_fit, lefty, leftx, righty, rightx, val
...
def project_lanelines(binary_warped, orig_img, left_fit, right_fit, dst_mask, src_mask):
    ...
    warped_inv = add_points(warped_inv, src)
    warped_inv = add_lines(warped_inv, src)
    road = warped_inv
    started = 1
...
def main_pipeline(input):
    ...
    left_fit, right_fit, lefty, leftx, righty, rightx, val = sliding_windows(binary_warped, nwindows=9)
...
def main():
    show_webcam(mirror=True,lane=True)

def show_webcam(mirror=True,lane=False):
    frames_counts = 1
    cap=cv2.VideoCapture(0)

    width = cap.set(3,1280)  # float
    height = cap.set(4,720) # float
    ...
    while True:
        ...
        if lane:
            main_pipeline(img)
            frame = cv2.addWeighted(img, 1, road, 0.5, 0)
    ...
    #     cv2.waitKey(12)
    #     finish=time.time()
    #     print('FPS:  ' + str(int(1/(finish-start+1))))
