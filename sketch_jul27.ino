...
#define L293N_ENA 6 // DIGITAL PIN 6
...
int curve=90;
const int SERVO     = 3; // DIGITAL PIN 3
int loop_i          = 0;
int angle           = 0;
Servo myservo;
...
void loop() {
 ...
 //   return;
    ...
    if(distance[0] < 10 || distance[3] < 10)
      car_stop();
    else if(str2.compareTo("고") == 0){
    //if(str2.compareTo("right") == 0){
      Serial.println("OK");
      car_go();
      flag = 1;
    }
    else if(str2.compareTo("뒤로") == 0){
      car_back();
      flag = 2;
    }
    else if(str2.compareTo("스탑") == 0){
      car_stop();
      flag = 0;
    }
    else if(flag == 1)
    ...
    if(str2.compareTo("왼쪽") == 0){  
    //if(str2.compareTo("LEFT") == 0){
      curve = curve - 30;
      myservo.write(curve);
      delay(50);
    }
    else if(str2.compareTo("오른쪽") == 0){
